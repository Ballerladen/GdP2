...
public class PlayList extends LinkedList<AudioFile> {
	...
	public void sort(SortCriterion order) {
		...
		AuthorComparator ac = null;
		TitleComparator tc = null;
		AlbumComparator alc = null;
		DurationComparator dc = null;

		switch (order) {
		case AUTHOR:
			ac = new AuthorComparator();
			Collections.sort(this, ac);
			break;
		case TITLE:
			tc = new TitleComparator();
			Collections.sort(this, tc);
			break;
		case ALBUM:
			alc = new AlbumComparator();
			Collections.sort(this, alc);
			break;
		case DURATION:
			...
			break;
		default:
			throw new RuntimeException("Couldn´t detect sort Criterion");
		}
